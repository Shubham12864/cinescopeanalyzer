...
class MovieService:
    ...
    async def _search_omdb_direct(self, query: str, limit: int) -> List[Movie]:
        ...
        try:
            ...
            load_dotenv()
            omdb_api_key = os.getenv('OMDB_API_KEY', '4977b044')
    ...
    # New methods for different movie sections
    async def get_popular_movies(self, limit: int = 12) -> List[Movie]:
        ...
        try:
            ...
            if movies:
                return [self._convert_dict_to_movie(movie) for movie in movies if self._convert_dict_to_movie(movie)]
            return []
        except Exception as e:
            self.logger.error(f"Error getting popular movies from database: {e}")
            return []
    
    async def fetch_and_store_popular_movies(self, limit: int = 12) -> List[Movie]:
        """Fetch popular movies from APIs and store in database"""
        try:
            self.logger.info("🔥 Fetching popular movies from APIs...")
            
            # Popular movie queries to search for
            popular_queries = [
                "avengers", "batman", "star wars", "lord of the rings", "marvel",
                "godfather", "inception", "dark knight", "matrix", "titanic",
                "shawshank redemption", "pulp fiction", "forrest gump", "gladiator"
            ]
            
            popular_movies = []
            
            for query in popular_queries[:8]:  # Limit queries to avoid rate limits
                try:
                    movies_data = await self.api_manager.search_movies(query, 2)
                    
                    for movie_data in movies_data:
                        if movie_data.get('source') in ['omdb_live', 'tmdb_live'] and movie_data.get('rating', 0) >= 7.0:
                            movie = Movie(
                                id=movie_data.get('id', f"popular_{len(popular_movies)}"),
                                title=movie_data.get('title', ''),
                                plot=movie_data.get('plot', ''),
                                rating=movie_data.get('rating', 0),
                                genre=movie_data.get('genre', []),
                            ...
                            movie_dict['category'] = 'popular'
                            movie_dict['last_updated'] = datetime.now().isoformat()
                    ...
                    self.logger.warning(f"Failed to fetch popular movies for '{query}': {e}")
                    continue
                    
                if len(popular_movies) >= limit:
                    break
            
            self.logger.info(f"✅ Fetched and stored {len(popular_movies)} popular movies")
    ...
    async def fetch_and_store_top_rated_movies(self, limit: int = 12) -> List[Movie]:
        """Fetch top-rated movies from APIs and store in database"""
        try:
            # Top-rated movie titles to search for
            top_rated_titles = [
                "The Shawshank Redemption", "The Godfather", "The Dark Knight",
                "Pulp Fiction", "The Lord of the Rings", "Schindler's List",
                "12 Angry Men", "The Godfather Part II", "The Good, the Bad and the Ugly",
            ...
            for title in top_rated_titles:
                try:
                    movies_data = await self.api_manager.search_movies(title, 1)
                    
                    if movies_data and len(movies_data) > 0:
                        movie_data = movies_data[0]
                        
                        if movie_data.get('rating', 0) >= 8.0:
                            movie = Movie(
                                id=movie_data.get('id', f"top_{len(top_movies)}"),
                                title=movie_data.get('title', ''),
                                plot=movie_data.get('plot', ''),
                                rating=movie_data.get('rating', 0),
                                genre=movie_data.get('genre', []),
